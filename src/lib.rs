...
#[no_mangle]
pub extern fn is_initialized() -> bool {
  unsafe {
    SYSTEM.is_some()
  }
}
...
fn get_pid_internal() -> Result<Pid, Box<dyn std::error::Error>> {
  ...
  unsafe {
    ...
    let proc_with_name = SYSTEM.as_mut().unwrap().processes_by_name(exe).into_iter();
...
// Get Pid as number
#[no_mangle]
pub extern fn get_pid() -> f64 {
  get_pid_internal().unwrap_or(Pid::from(0)).as_u32() as f64
}

// Get memory maximum for the system
#[no_mangle]
pub extern fn get_memory_max() -> f64 {
  if !is_initialized() {
    return -1.0;
  }

  unsafe {
    SYSTEM.as_mut().unwrap().total_memory() as f64
  }
}

// Get CPU frequency
#[no_mangle]
pub extern fn get_cpu_frequency() -> f64 {
  if !is_initialized() {
    return -1.0;
  }
...
pub extern "C" fn get_cpu_name() -> *mut c_char {
  ...
  unsafe {
    ...
    CString::new(cpu_name).unwrap_or(CString::new("").unwrap()).into_raw()
  }
}
...
pub extern "C" fn get_cpu_brand() -> *mut c_char {
  ...
  unsafe {
    ...
    CString::new(cpu_brand).unwrap_or(CString::new("").unwrap()).into_raw()
  }
}
...
pub extern "C" fn get_cpu_vendor_id() -> *mut c_char {
  ...
  unsafe {
    ...
    CString::new(cpu_vendor_id).unwrap_or(CString::new("").unwrap()).into_raw()
  }
}

// Get memory used for the system
#[no_mangle]
pub extern fn sys_memory_used() -> f64 {
  if !is_initialized() {
    return -1.0;
  }

  unsafe {
    SYSTEM.as_mut().unwrap().used_memory() as f64
  }
}

// Get memory used for the process
#[no_mangle]
pub extern fn proc_memory_used() -> f64 {
  if !is_initialized() {
    return -1.0;
  }
...
#[no_mangle]
pub extern fn sys_cpu_usage() -> f64 {
  if !is_initialized() {
    return -1.0;
  }
...
#[no_mangle]
pub extern fn proc_cpu_usage() -> f64 {
  if !is_initialized() {
    return -1.0;
  }
